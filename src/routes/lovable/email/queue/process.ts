        ...
        for (const queue of ['auth_emails', 'transactional_emails']) {
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            try {
              ...
              // Log "handed to provider" — provider accepted the API call.
              // This is NOT a delivery confirmation. Delivery state (bounced /
              // complained / suppressed) arrives later via the suppression webhook;
              // absence of a follow-up event means delivery is unconfirmed.
              await supabase.from('email_send_log').insert({
                message_id: payload.message_id,
